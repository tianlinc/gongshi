...
修复 v1.1.10 去掉 AppId 花括号后覆盖安装导致桌面快捷方式重复、安装目录文件共存的问题。

### 修复

- **AppId 回退**：`setup.iss` 的 `AppId` 从纯字符串 `A8F3C2B1-...` 回退为 `{{A8F3C2B1-...}}`（双花括号），与 v1.1.7-v1.1.9 时代保持一致。v1.1.10 去掉花括号后 Inno Setup 将其识别为全新应用，不会触发 IsUpgrade，导致新旧两套注册表项、快捷方式、文件并存
- **GetUninstallString 双路径查找**：先查花括号格式 `{GUID}_is1`（存量），再回退查纯字符串格式（v1.1.10 遗留），确保无论旧版本是哪种格式都能正确匹配卸载
- **KNOWN_APP_ID 双值兼容**：`_desktop_common.py` 同时保存花括号和纯字符串两个 AppId，依序查找
- **InstallDelete 补充**：新增 `{autodesktop}\IEI Timer Faster.lnk` 清理，兜底删除旧桌面快捷方式
